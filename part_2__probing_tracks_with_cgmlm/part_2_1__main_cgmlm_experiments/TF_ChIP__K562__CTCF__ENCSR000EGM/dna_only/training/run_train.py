...
from local_utils.lightning_models.masked_unet.dna_input_dna_output import LightningModel
...
def main(config_dict):
    ...
    valid_data_config.max_shift = 100
...
if __name__ == "__main__":
    ...
    config_dict.task.cell_type = "K562"  # Match your dataset
    ...
    config_dict.data.peaks_zarr_path = osp.join(
        ...
        "workspace",
        "data",
        "TF_ChIP",  # Changed from ATAC
        "K562__CTCF__ENCSR000EGM",  # Updated for K562 TF_ChIP
        "preprocessed",
        "zarr_datasets",
    ...
    # Main experiment tracks with +/- channels
    # NOTE: DNA-only training: track dataset are needed for dataset but ignored by lightning model
    ...
    config_dict.data.peak_to_nonpeak_ratio = 3  # Changed from 10
    ...
    config_dict.data.max_shift = 100  # Already correct
    ...
    config_dict.lightning.devices = [1] if torch.cuda.is_available() else 1  # Use 1 CPU device instead of None
    ...
    config_dict.lightning.batch_size = (512 - 128) * num_devices
