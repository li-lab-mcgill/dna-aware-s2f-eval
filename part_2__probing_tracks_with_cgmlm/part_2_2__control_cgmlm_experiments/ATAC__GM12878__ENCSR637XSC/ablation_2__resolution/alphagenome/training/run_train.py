...
if __name__ == "__main__":
    ...
    config_dict.task.ablation_type = "ablation_2__resolution"
    ...
    config_dict.batch_preparer.bin_width = 8  # no binning
    config_dict.batch_preparer.equalize_counts = True
    ...
    config_dict.lightning.devices = [6] if torch.cuda.is_available() else 1  # Use 1 CPU device instead of None
    ...
    config_dict.task.experiment_dir = osp.join(
        ...
        mask_dir,
        f"seed_{config_dict.task.seed}__resolution_{config_dict.batch_preparer.bin_width}"
    )
