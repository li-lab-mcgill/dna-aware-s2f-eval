    ...
    for fold_name in config_dict.task.fold_names:
        ...
        for split_name in config_dict.task.split_names:
            ...
            for replicate_id in config_dict.eval.replicate_ids:
                ...
                # Loop over each model and its MATCHING window size
                for model_name, checkpoint_path in config_dict.task.model_names_and_checkpoints.items():
                    window_size = int(model_name)  # Model name IS the window size
                    mask_type = "100pct"  # Fixed mask type for locality ablation
                    print(f"\n-- Model with window size {window_size} (mask type: {mask_type}) --")

                    if checkpoint_path is None:
                        print(f"Skipping {model_name} because checkpoint path is None")
                        continue
                    
                    # Load model for this window size
                    print(f"Loading model with window size {window_size} from {checkpoint_path}")
                    ...
                    # Create batch preparer with tiling for fair comparison
                    # Always crop to 512bp, then tile into window_size chunks
                    batch_preparer = BatchPreparer(
                        region_lengths=(512,),          # FIXED: always crop to 512bp
                        tile_length=window_size,        # TILE: into window_size chunks
                        bin_width=config_dict.batch_preparer.bin_width,
                    ...
                    pred_nonpeaks_track_dataset_paths = \
                        [
                            [path.format(fold_name) if '{}' in path else path for path in track_path]
                                for track_path in config_dict.data.predicted_nonpeaks_track_dataset_paths
                        ]
                    pred_peaks_track_dataset_paths = \
                        [
                            [path.format(fold_name) if '{}' in path else path for path in track_path]
                                for track_path in config_dict.data.predicted_peaks_track_dataset_paths
                        ]
                    
                    # Create dataset with fixed mask type (100pct for locality ablation)
                    dataset = create_dataset(
                        ...
                        predicted_peaks_track_dataset_paths=pred_peaks_track_dataset_paths,  # ← Format this
                        predicted_nonpeaks_track_dataset_paths=pred_nonpeaks_track_dataset_paths,  # ← Format this
                        ...
                        mask_type_str=mask_type,  # Fixed at mask_100pct
...
if __name__ == "__main__":
    ...
    config_dict.task.model_names_and_checkpoints = {
        "16": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_16/lightning_logs/version_0/checkpoints/best_val_loss-epoch=26-step=12879.ckpt",
        "32": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_32/lightning_logs/version_0/checkpoints/best_val_loss-epoch=33-step=16218.ckpt",
        "64": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_64/lightning_logs/version_0/checkpoints/best_val_loss-epoch=55-step=26712.ckpt",
        "128": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_128/lightning_logs/version_0/checkpoints/best_val_loss-epoch=47-step=22896.ckpt",
        "256": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_256/lightning_logs/version_0/checkpoints/best_val_loss-epoch=46-step=22419.ckpt",
        "512": "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/manuscript_experiments/section_2__ConditionalLM_controlled_experiments/workspace/runs/ATAC/GM12878__ENCSR637XSC/ablation_5__locality/bpnet__predicted/fold_0/mask_100pct/seed_42__local_window_512/lightning_logs/version_0/checkpoints/best_val_loss-epoch=59-step=28620.ckpt",
    }
    ...
    # Batch preparer configuration template (instantiated per window size in the loop)
    # Each model evaluation will create its own BatchPreparer with matching region_lengths
    config_dict.batch_preparer = ConfigDict()
    config_dict.batch_preparer.bin_width = 1          # no binning
    config_dict.batch_preparer.equalize_counts = True # equalize counts over visible rows
    ...
    config_dict.track_key_in_batch = "pred_tracks"
