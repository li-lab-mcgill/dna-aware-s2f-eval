...
if __name__ == "__main__":
    ...
    config_dict.task.ablation_type = "ablation_3__locality"
    ...
    # batch preparer configuration (crop-based locality with fixed K)
    region_length = 16  # set to one of {16, 64, 128, 256, 512}
    config_dict.batch_preparer = ConfigDict()
    config_dict.batch_preparer.region_lengths = (region_length,)
    config_dict.batch_preparer.bin_width = 1
    config_dict.batch_preparer.equalize_counts = True  # shape-only for predicted tracks
    ...
    config_dict.task.experiment_dir = osp.join(
        ...
        mask_dir,
        f"seed_{config_dict.task.seed}__local_window_{region_length}"
    )
