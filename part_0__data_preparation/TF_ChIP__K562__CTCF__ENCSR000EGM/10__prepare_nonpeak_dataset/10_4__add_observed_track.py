...
def create_observed_track_dataset(config_dict: ConfigDict, fold_name: str):
    """Add observed track datasets to existing fold-specific zarr file"""
    
    zarr_path = os.path.join(config_dict.output_dir, f"nonpeaks.{fold_name}.zarr")
    
    # Setup logging
    print(f"=== Adding observed track datasets for {fold_name} ===")
    ...
    print(f"✓ Successfully added observed track datasets for {fold_name}")
...
def main():
    ...
    for fold_name in fold_names:
        ...
        create_observed_track_dataset(config_dict, fold_name)
        print(f"✓ Completed {fold_name}")
    
    print(f"✓ Completed adding observed track datasets for all folds")
