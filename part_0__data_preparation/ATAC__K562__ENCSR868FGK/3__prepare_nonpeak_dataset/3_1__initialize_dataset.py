...
def create_split_indices(regions_df, config_dict):
    """
    Extract existing train/val/test split indices using single splits column
    """
    # Extract indices directly from splits column (None values automatically excluded)
    train_indices = regions_df[regions_df['splits'] == 'training']['region_idx'].tolist()
    val_indices = regions_df[regions_df['splits'] == 'validation']['region_idx'].tolist()
    test_indices = regions_df[regions_df['splits'] == 'test']['region_idx'].tolist()
...
def save_auxiliary_files(zarr_path, regions_df, train_indices, val_indices, test_indices):
...
def main(config_dict):
    ...
    for fold_name in config_dict.data.fold_names:
        ...
        regions_file = os.path.join(config_dict.data.data_dir, f"nonpeaks.{fold_name}.tsv")
        regions_df = prepare_regions_df(regions_file, config_dict)
        
        # 2. Extract existing train/val/test split indices
        train_indices, val_indices, test_indices = create_split_indices(regions_df, config_dict)

        # 3. Initialize empty zarr dataset
        zarr_path = initialize_zarr_dataset(regions_df, config_dict, f"nonpeaks.{fold_name}.zarr")

        # 4. Save auxiliary files (regions_df and split indices)
        save_auxiliary_files(zarr_path, regions_df, train_indices, val_indices, test_indices)
