...
def validate_observed_track_dataset(config_dict: ConfigDict):
    ...
    if dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {dataset_name} does not exist in zarr file. Run 2_4 first to create it.")
    ...
    for i, row in tqdm(regions_df.iterrows(), total=N, desc="validating regions"):
        ...
        # Re-extract from BigWig using same logic as 2_4
        try:
...
def main():
    ...
    print("Validating observed track dataset...")
