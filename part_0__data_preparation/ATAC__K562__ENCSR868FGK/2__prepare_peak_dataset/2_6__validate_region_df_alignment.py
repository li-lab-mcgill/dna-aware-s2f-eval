...
def validate_peaks_data(config_dict: ConfigDict):
    """Validate both DNA sequences and observed tracks for peaks by comparing zarr data with fresh extraction"""
    print(f"\n=== Validating peaks.all_folds.zarr ===")
    
    zarr_path = os.path.join(config_dict.output_dir, "peaks.all_folds.zarr")
    ...
    if 'splits' in regions_df.columns:
        fold_counts = regions_df['splits'].value_counts()
        print(f"\n🔍 FOLD DISTRIBUTION:")
        for fold, count in fold_counts.items():
            print(f"  {fold}: {count:,} regions")

    if all_passed:
        print(f"\n✅ PEAKS VALIDATION PASSED - All data matches perfectly!")
    else:
        if track_int_match:
            print(f"\n⚠️  PEAKS VALIDATION: Integer values match but float precision differs")
            print(f"   This is likely due to floating-point precision and may not affect training")
        else:
            print(f"\n❌ PEAKS VALIDATION FAILED - Found significant mismatches in stored data!")
...
def main():
    ...
    # Validate peaks dataset
    print("Starting comprehensive DNA + Track validation for peaks dataset...")
    
    validation_passed = validate_peaks_data(config_dict)
    
    print(f"\n{'='*60}")
    if validation_passed:
        print(f"🎉 PEAKS VALIDATION PASSED!")
