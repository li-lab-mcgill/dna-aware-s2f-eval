...
def prepare_regions_df(regions_file, config_dict):
    ...
    """
    # Load regions (already has train/val/test splits)
...
def create_split_indices(regions_df, config_dict):
    """
    Extract existing train/val/test split indices for each fold separately
    """
    # Get fold columns from the dataframe
    fold_columns = [col for col in regions_df.columns if col.endswith('__splits')]
    
    all_split_indices = {}
    
    for fold_col in fold_columns:
        fold_name = fold_col.replace('__splits', '')  # e.g., 'fold_0__splits' -> 'fold_0'
        
        # Extract indices directly from fold column (None values automatically excluded)
        train_indices = regions_df[regions_df[fold_col] == 'training']['region_idx'].tolist()
        val_indices = regions_df[regions_df[fold_col] == 'validation']['region_idx'].tolist()
        test_indices = regions_df[regions_df[fold_col] == 'test']['region_idx'].tolist()
        
        # Store with the naming convention you want
...
def chromosome_dna_worker(
    ...
    try:
        ...
        root["dna_summitcentered2114bp_symmetric500bpshift"][chrom_indices] = input_buf
        root["dna_summitcentered2114bp_symmetric750bpshift"][chrom_indices] = input_buf_for_tile
        root["dna_summitcentered1000bp_symmetric500bpshift"][chrom_indices] = output_buf
...
def main(config_dict):
    ...
    regions_file = os.path.join(config_dict.data.data_dir, f"peaks.all_folds.tsv")
    regions_df = prepare_regions_df(regions_file, config_dict)
    
    # 2. Extract existing train/val/test split indices
    all_split_indices = create_split_indices(regions_df, config_dict)

    # 3. Initialize empty zarr dataset
    zarr_path = initialize_zarr_dataset(regions_df, config_dict, "peaks.all_folds.zarr")
