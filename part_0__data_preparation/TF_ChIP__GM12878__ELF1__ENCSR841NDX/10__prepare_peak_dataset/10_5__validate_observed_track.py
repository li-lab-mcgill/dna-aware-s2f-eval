...
def validate_observed_track_dataset(config_dict: ConfigDict):
    ...
    # Setup logging
    print(f"=== Validating observed track datasets ===")
    ...
    if main_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {main_dataset_name} does not exist in zarr file. Run 10_4 first to create it.")
    if control_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {control_dataset_name} does not exist in zarr file. Run 10_4 first to create it.")
    if main_input_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {main_input_dataset_name} does not exist in zarr file. Run 10_4 first to create it.")
    if control_input_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {control_input_dataset_name} does not exist in zarr file. Run 10_4 first to create it.")
    ...
    stored_main_input_dataset = root["observed_tracks"][main_input_dataset_name]
    stored_control_input_dataset = root["observed_tracks"][control_input_dataset_name]
    print(f"Loaded stored datasets: {main_dataset_name}, {control_dataset_name}, {main_input_dataset_name}, {control_input_dataset_name}")
    ...
    reextracted_main_input_values = np.empty((N, config_dict.shift_extended_input_region_for_tile_len, 2), dtype=np.float32)
    reextracted_control_input_values = np.empty((N, config_dict.shift_extended_input_region_for_tile_len, 2), dtype=np.float32)
    ...
    # Validation statistics for input tracks
    main_input_nan_counts_per_region = []
    main_input_regions_with_length_mismatch = 0
    main_input_regions_with_no_data = 0
    main_input_regions_with_mismatches = 0
    main_input_total_mismatched_positions = 0
    main_input_max_absolute_diff = 0.0
    
    control_input_nan_counts_per_region = []
    control_input_regions_with_length_mismatch = 0
    control_input_regions_with_no_data = 0
    control_input_regions_with_mismatches = 0
    control_input_total_mismatched_positions = 0
    control_input_max_absolute_diff = 0.0
    ...
    for i, row in tqdm(regions_df.iterrows(), total=N, desc="validating regions"):
        ...
        # Re-extract main track from BigWig using same logic as 10_4
        try:
        ...
        # Re-extract control track from BigWig using same logic as 10_4
        try:
        ...
        # Process input length tracks (3614bp) - ADD THIS AFTER THE CONTROL TRACK PROCESSING
        input_start = int(row['input_extended_start_for_tile'])
        input_end = int(row['input_extended_end_for_tile'])
        
        # Re-extract main input track
        try:
            ...
            if main_plus_input is None:
                ...
                main_input_regions_with_no_data += 1
            else:
                main_plus_input = np.array(main_plus_input, dtype=np.float32)
                if len(main_plus_input) != config_dict.shift_extended_input_region_for_tile_len:
                    main_input_regions_with_length_mismatch += 1
                    raise ValueError(f"Main input plus length mismatch for {chrom}:{input_start}-{input_end}, expected {config_dict.shift_extended_input_region_for_tile_len}, got {len(main_plus_input)}")
                main_plus_input = np.nan_to_num(main_plus_input, nan=0.0)
            ...
            if main_minus_input is None:
                ...
                main_minus_input = np.array(main_minus_input, dtype=np.float32)
                if len(main_minus_input) != config_dict.shift_extended_input_region_for_tile_len:
                    main_input_regions_with_length_mismatch += 1
                    raise ValueError(f"Main input minus length mismatch for {chrom}:{input_start}-{input_end}, expected {config_dict.shift_extended_input_region_for_tile_len}, got {len(main_minus_input)}")
                main_minus_input = np.nan_to_num(main_minus_input, nan=0.0)
            
            reextracted_main_input_values[region_idx, :, 0] = main_plus_input
            reextracted_main_input_values[region_idx, :, 1] = main_minus_input
            
            main_input_nan_count = np.sum(np.isnan(main_plus_input)) + np.sum(np.isnan(main_minus_input))
            main_input_nan_counts_per_region.append(main_input_nan_count)
            
        except Exception as e:
            print(f"Error processing main input track for region {chrom}:{input_start}-{input_end}: {e}")
            reextracted_main_input_values[region_idx, :, :] = 0.0
            main_input_nan_counts_per_region.append(config_dict.shift_extended_input_region_for_tile_len * 2)
        
        # Re-extract control input track  
        try:
            ...
            if control_plus_input is None:
                ...
                control_input_regions_with_no_data += 1
            else:
                control_plus_input = np.array(control_plus_input, dtype=np.float32)
                if len(control_plus_input) != config_dict.shift_extended_input_region_for_tile_len:
                    control_input_regions_with_length_mismatch += 1
                    raise ValueError(f"Control input plus length mismatch for {chrom}:{input_start}-{input_end}, expected {config_dict.shift_extended_input_region_for_tile_len}, got {len(control_plus_input)}")
                control_plus_input = np.nan_to_num(control_plus_input, nan=0.0)
            ...
            if control_minus_input is None:
                ...
                control_minus_input = np.array(control_minus_input, dtype=np.float32)
                if len(control_minus_input) != config_dict.shift_extended_input_region_for_tile_len:
                    control_input_regions_with_length_mismatch += 1
                    raise ValueError(f"Control input minus length mismatch for {chrom}:{input_start}-{input_end}, expected {config_dict.shift_extended_input_region_for_tile_len}, got {len(control_minus_input)}")
                control_minus_input = np.nan_to_num(control_minus_input, nan=0.0)
            
            reextracted_control_input_values[region_idx, :, 0] = control_plus_input
            reextracted_control_input_values[region_idx, :, 1] = control_minus_input
            
            control_input_nan_count = np.sum(np.isnan(control_plus_input)) + np.sum(np.isnan(control_minus_input))
            control_input_nan_counts_per_region.append(control_input_nan_count)
            
        except Exception as e:
            print(f"Error processing control input track for region {chrom}:{input_start}-{input_end}: {e}")
            reextracted_control_input_values[region_idx, :, :] = 0.0
            control_input_nan_counts_per_region.append(config_dict.shift_extended_input_region_for_tile_len * 2)
    ...
    # Add input track validation variables
    main_input_all_match = True
    control_input_all_match = True
    main_input_non_integer_values_count = 0
    control_input_non_integer_values_count = 0
    
    # ADD FULL VALIDATION FOR INPUT TRACKS AFTER THE EXISTING CONTROL TRACK VALIDATION
    
    print("=== Validating Main Input Track ===")
    for chunk_start in tqdm(range(0, N, chunk_size), desc="comparing main input track chunks"):
        chunk_end = min(chunk_start + chunk_size, N)
        
        stored_main_input_chunk = stored_main_input_dataset[chunk_start:chunk_end]
        reextracted_main_input_chunk = reextracted_main_input_values[chunk_start:chunk_end]
        
        stored_main_input_is_int = np.isclose(stored_main_input_chunk, np.round(stored_main_input_chunk))
        reextracted_main_input_is_int = np.isclose(reextracted_main_input_chunk, np.round(reextracted_main_input_chunk))
        
        non_integer_stored_main_input = np.sum(~stored_main_input_is_int)
        non_integer_reextracted_main_input = np.sum(~reextracted_main_input_is_int)
        main_input_non_integer_values_count += non_integer_stored_main_input + non_integer_reextracted_main_input
        
        if non_integer_stored_main_input > 0 or non_integer_reextracted_main_input > 0:
            print(f"Warning: Found non-integer values in main input track chunk {chunk_start}-{chunk_end}")
            print(f"  Non-integer stored values: {non_integer_stored_main_input}")
            print(f"  Non-integer reextracted values: {non_integer_reextracted_main_input}")
        
        stored_main_input_chunk_int = np.round(stored_main_input_chunk).astype(np.int32)
        reextracted_main_input_chunk_int = np.round(reextracted_main_input_chunk).astype(np.int32)
        
        exact_main_input_match = np.array_equal(stored_main_input_chunk_int, reextracted_main_input_chunk_int)
        
        if not exact_main_input_match:
            main_input_all_match = False
            main_input_mismatch_mask = (stored_main_input_chunk_int != reextracted_main_input_chunk_int)
            chunk_main_input_mismatched_positions = np.sum(main_input_mismatch_mask)
            main_input_total_mismatched_positions += chunk_main_input_mismatched_positions
            
            main_input_diff = np.abs(stored_main_input_chunk - reextracted_main_input_chunk)
            chunk_main_input_max_diff = np.max(main_input_diff)
            main_input_max_absolute_diff = max(main_input_max_absolute_diff, chunk_main_input_max_diff)
            
            main_input_regions_with_mismatches_in_chunk = np.sum(np.any(main_input_mismatch_mask, axis=(1, 2)))
            main_input_regions_with_mismatches += main_input_regions_with_mismatches_in_chunk
    
    print("=== Validating Control Input Track ===")
    for chunk_start in tqdm(range(0, N, chunk_size), desc="comparing control input track chunks"):
        chunk_end = min(chunk_start + chunk_size, N)
        
        stored_control_input_chunk = stored_control_input_dataset[chunk_start:chunk_end]
        reextracted_control_input_chunk = reextracted_control_input_values[chunk_start:chunk_end]
        
        stored_control_input_is_int = np.isclose(stored_control_input_chunk, np.round(stored_control_input_chunk))
        reextracted_control_input_is_int = np.isclose(reextracted_control_input_chunk, np.round(reextracted_control_input_chunk))
        
        non_integer_stored_control_input = np.sum(~stored_control_input_is_int)
        non_integer_reextracted_control_input = np.sum(~reextracted_control_input_is_int)
        control_input_non_integer_values_count += non_integer_stored_control_input + non_integer_reextracted_control_input
        
        if non_integer_stored_control_input > 0 or non_integer_reextracted_control_input > 0:
            print(f"Warning: Found non-integer values in control input track chunk {chunk_start}-{chunk_end}")
            print(f"  Non-integer stored values: {non_integer_stored_control_input}")
            print(f"  Non-integer reextracted values: {non_integer_reextracted_control_input}")
        
        stored_control_input_chunk_int = np.round(stored_control_input_chunk).astype(np.int32)
        reextracted_control_input_chunk_int = np.round(reextracted_control_input_chunk).astype(np.int32)
        
        exact_control_input_match = np.array_equal(stored_control_input_chunk_int, reextracted_control_input_chunk_int)
        
        if not exact_control_input_match:
            control_input_all_match = False
            control_input_mismatch_mask = (stored_control_input_chunk_int != reextracted_control_input_chunk_int)
            chunk_control_input_mismatched_positions = np.sum(control_input_mismatch_mask)
            control_input_total_mismatched_positions += chunk_control_input_mismatched_positions
            
            control_input_diff = np.abs(stored_control_input_chunk - reextracted_control_input_chunk)
            chunk_control_input_max_diff = np.max(control_input_diff)
            control_input_max_absolute_diff = max(control_input_max_absolute_diff, chunk_control_input_max_diff)
            
            control_input_regions_with_mismatches_in_chunk = np.sum(np.any(control_input_mismatch_mask, axis=(1, 2)))
            control_input_regions_with_mismatches += control_input_regions_with_mismatches_in_chunk
    
    # ADD INPUT TRACK RESULTS TO THE VALIDATION RESULTS SECTION
    
    # Main input track results
    print(f"\n=== Main Input Track Results ===")
    print(f"Regions with no data: {main_input_regions_with_no_data}")
    print(f"Regions with length mismatch: {main_input_regions_with_length_mismatch}")
    
    if main_input_all_match:
        print(f"✅ MAIN INPUT TRACK VALIDATION PASSED: All re-extracted values match stored values perfectly!")
    else:
        print(f"❌ MAIN INPUT TRACK VALIDATION FAILED: Found mismatches between re-extracted and stored values")
        print(f"  Regions with mismatches: {main_input_regions_with_mismatches:,}")
        print(f"  Total mismatched positions: {main_input_total_mismatched_positions:,}")
        print(f"  Maximum absolute difference: {main_input_max_absolute_diff:.8f}")
        
        total_main_input_positions = N * config_dict.shift_extended_input_region_for_tile_len * 2
        main_input_mismatch_percentage = (main_input_total_mismatched_positions / total_main_input_positions) * 100
        print(f"  Percentage of positions with mismatches: {main_input_mismatch_percentage:.6f}%")
    
    # Control input track results
    print(f"\n=== Control Input Track Results ===")
    print(f"Regions with no data: {control_input_regions_with_no_data}")
    print(f"Regions with length mismatch: {control_input_regions_with_length_mismatch}")
    
    if control_input_all_match:
        print(f"✅ CONTROL INPUT TRACK VALIDATION PASSED: All re-extracted values match stored values perfectly!")
    else:
        print(f"❌ CONTROL INPUT TRACK VALIDATION FAILED: Found mismatches between re-extracted and stored values")
        print(f"  Regions with mismatches: {control_input_regions_with_mismatches:,}")
        print(f"  Total mismatched positions: {control_input_total_mismatched_positions:,}")
        print(f"  Maximum absolute difference: {control_input_max_absolute_diff:.8f}")
        
        total_control_input_positions = N * config_dict.shift_extended_input_region_for_tile_len * 2
        control_input_mismatch_percentage = (control_input_total_mismatched_positions / total_control_input_positions) * 100
        print(f"  Percentage of positions with mismatches: {control_input_mismatch_percentage:.6f}%")
    ...
    return main_all_match and control_all_match and main_input_all_match and control_input_all_match
...
def main():
    ...
    # BigWig file paths (same as 10_4)
    ...
    # Validate observed track dataset
    print("Validating observed track datasets...")
    
    validation_passed = validate_observed_track_dataset(config_dict)
    
    if validation_passed:
        print(f"✅ Validation completed successfully - all data matches!")
    else:
