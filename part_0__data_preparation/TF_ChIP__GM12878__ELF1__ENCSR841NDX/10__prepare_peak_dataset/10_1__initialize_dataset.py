...
def prepare_regions_df(regions_file, config_dict):
    ...
    # add columns for MACS2 peak columns
    regions_df.columns = \
...
def create_split_indices(regions_df, config_dict):
    """
    Extract train/val/test split indices for each fold based on chromosome assignments
    """
    all_split_indices = {}
    
    for fold_name in config_dict.data.fold_names:
        # Load the splits file for this fold
        splits_file = os.path.join(config_dict.data.splits_dir, f"{fold_name}.json")
        with open(splits_file, 'r') as f:
            splits = json.load(f)
        
        # Extract indices for each split based on chromosome assignments
        ...
        # Store with the naming convention
...
def main(config_dict):
    ...
    regions_file = os.path.join(config_dict.data.data_dir, f"peaks.bed")
    regions_df = prepare_regions_df(regions_file, config_dict)
    
    # 2. Extract train/val/test split indices
