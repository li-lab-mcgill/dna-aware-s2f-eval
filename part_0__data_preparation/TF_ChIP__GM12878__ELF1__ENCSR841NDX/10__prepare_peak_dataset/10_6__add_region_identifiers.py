...
def main():
    """Main function to add identifiers to peaks dataset"""
    ...
    # Add identifiers to peaks dataset
    zarr_path = os.path.join(config_dict.output_dir, "peaks.all_folds.zarr")
    if not os.path.exists(zarr_path):
        print(f"Warning: Zarr dataset not found: {zarr_path}")
        return
    
    add_identifiers_to_regions_df(zarr_path)
    print("✓ Completed adding identifiers to peaks dataset")
