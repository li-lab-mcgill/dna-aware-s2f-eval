...
def validate_observed_track_dataset(config_dict: ConfigDict, fold_name: str):
    ...
    if main_input_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {main_input_dataset_name} does not exist in zarr file. Run 11_4 first to create it.")
    if control_input_dataset_name not in root["observed_tracks"]:
        raise ValueError(f"Dataset {control_input_dataset_name} does not exist in zarr file. Run 11_4 first to create it.")
    ...
    for i, row in tqdm(regions_df.iterrows(), total=N, desc="processing input tracks"):
        region_idx = row['region_idx']
        chrom = row['Chromosome']
    ...
    for chunk_start in tqdm(range(0, N, chunk_size), desc="comparing main input track chunks"):
        ...
        reextracted_main_input_chunk = main_input_track_values[chunk_start:chunk_end]
    ...
    for chunk_start in tqdm(range(0, N, chunk_size), desc="comparing control input track chunks"):
        ...
        reextracted_control_input_chunk = control_input_track_values[chunk_start:chunk_end]
    ...
    # Main input track data
    print(f"\nMain input track re-extracted data statistics:")
    print(f"  Min value: {np.min(main_input_track_values):.6f}")
    print(f"  Max value: {np.max(main_input_track_values):.6f}")
    print(f"  Mean value: {np.mean(main_input_track_values):.6f}")
    print(f"  Non-zero positions: {np.count_nonzero(main_input_track_values):,}")

    # Control input track data
    print(f"\nControl input track re-extracted data statistics:")
    print(f"  Min value: {np.min(control_input_track_values):.6f}")
    print(f"  Max value: {np.max(control_input_track_values):.6f}")
    print(f"  Mean value: {np.mean(control_input_track_values):.6f}")
    print(f"  Non-zero positions: {np.count_nonzero(control_input_track_values):,}")
