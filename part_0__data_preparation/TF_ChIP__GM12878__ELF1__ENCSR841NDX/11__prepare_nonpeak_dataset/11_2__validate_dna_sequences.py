...
def validate_zarr_dataset(config_dict: ConfigDict, fold_name: str):
    """Validate a fold-specific zarr dataset by comparing with fresh extraction"""
    ...
    actual_input = zarr_store["reference_dna"]["dna_summitcentered2114bp_symmetric500bpshift"][:]
    actual_input_750 = zarr_store["reference_dna"]["dna_summitcentered2114bp_symmetric750bpshift"][:]
    actual_output = zarr_store["reference_dna"]["dna_summitcentered1000bp_symmetric500bpshift"][:]
...
def main():
    # Configuration - match 11_1 exactly
    ...
    print("Starting DNA sequence validation for all nonpeak folds...")
    ...
    for fold_name in config_dict.data.fold_names:
        ...
        if os.path.exists(zarr_path):
            try:
                validate_zarr_dataset(config_dict, fold_name)
            except Exception as e:
                print(f"❌ Validation failed for {fold_name}: {e}")
                all_passed = False
        else:
            print(f"⚠️  Zarr file not found for {fold_name}: {zarr_path}")
            all_passed = False
