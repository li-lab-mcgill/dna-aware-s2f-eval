...
if __name__ == "__main__":
    ...
    config_dict.data.fold_specific_peak_region_template = os.path.join(
        ...
        "ENCFF971WEQ",
        "{}", # fold name
        "peaks.{}set.{}.ENCSR637XSC.bed.gz") # NOTE: first {} is fold name, second is split name, third is fold name
    config_dict.data.fold_specific_nonpeak_region_template = os.path.join(
        config_dict.data.data_dir,
        "ENCFF971WEQ",
        "{}", # fold name
        "nonpeaks.{}set.{}.ENCSR637XSC.bed.gz") # NOTE: first {} is fold name, second is split name, third is fold name
