...
def create_predicted_track_dataset(config_dict: ConfigDict):
    ...
    # Get the target group and dataset name - save directly to correct location
    target_group = root["fold_agnostic__model_based_tracks"]
    ...
    if dataset_name in target_group:
        print(f"Dataset {dataset_name} already exists in fold_agnostic__model_based_tracks, will overwrite if allowed")
        pdb.set_trace()
    ...
    print(f"Created dataset: fold_agnostic__model_based_tracks/{dataset_name}")
    ...
    # Update attributes
    root.attrs[f'{dataset_name}_predicted_track_dataset'] = f"fold_agnostic__model_based_tracks/{dataset_name}"
    ...
    print(f"✓ Successfully added predicted track dataset to fold_agnostic__model_based_tracks")
...
def main():
    ...
    create_predicted_track_dataset(config_dict)
