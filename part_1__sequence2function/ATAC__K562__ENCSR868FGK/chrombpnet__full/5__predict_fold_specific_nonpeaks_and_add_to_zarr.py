...
def load_models_for_fold(config_dict, fold_name):
    ...
    # 1. Load full ChromBPNet model using both bias and accessibility components
    models['chrombpnet_full'] = ChromBPNet.from_chrombpnet(
        bias_model=fold_model_filepaths['bias_scaled'],
        accessibility_model=fold_model_filepaths['chrombpnet_nobias'],
        name=f"K562_ENCSR868FGK_ChromBPNet_{fold_name}"
    )
...
def create_predicted_track_dataset(fold_name: str, config_dict: ConfigDict):
    ...
    # CHANGE 1: Use fold-specific nonpeaks zarr file
    zarr_path = os.path.join(config_dict.output_dir, f"nonpeaks.{fold_name}.zarr")
    
    # Setup logging
    print(f"=== Adding predicted track datasets to {fold_name} ===")
...
def main():
    ...
    config_dict.model.model_filepaths = {
        "bias_scaled": os.path.join(config_dict.model.model_dir, "ENCFF984RAF", "{}", "model.bias_scaled.{}.ENCSR868FGK.h5"),
        "chrombpnet_nobias": os.path.join(config_dict.model.model_dir, "ENCFF984RAF", "{}", "model.chrombpnet_nobias.{}.ENCSR868FGK.h5")
    }
