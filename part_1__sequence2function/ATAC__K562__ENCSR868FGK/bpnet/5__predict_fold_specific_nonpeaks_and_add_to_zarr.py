...
def load_models_for_fold(config_dict, fold_name):
    ...
    # 2. Load ChromBPNet nobias model as BPNet (accessibility model)
    print("Loading BPNet model...")
    models['bpnet'] = BPNet.from_chrombpnet(fold_model_filepaths['bpnet'])
...
def create_predicted_track_dataset(fold_name: str, config_dict: ConfigDict):
    ...
    # device setup
    device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")  # Changed from cuda:1 to avoid conflict
...
def main():
    ...
    config_dict.model.model_filepaths = {
        "bpnet": os.path.join(config_dict.model.model_dir, "bpnet", "{}", "models", "chrombpnet_nobias.h5"),
    }
