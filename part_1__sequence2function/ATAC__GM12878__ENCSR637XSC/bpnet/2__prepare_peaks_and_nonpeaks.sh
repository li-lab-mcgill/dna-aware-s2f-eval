...
INPUT_DIR="$BASE_DIR/ATAC/GM12878__ENCSR637XSC/chrombpnet_annotation/ENCFF971WEQ"
OUTPUT_DIR="$BASE_DIR/ATAC/GM12878__ENCSR637XSC/preprocessed/chrombpnet_annotation__regions"
mkdir -p "$OUTPUT_DIR"

# peaks are .bed.gz files unzip them to a target directory
BED_FILE="$INPUT_DIR/peaks.all_input_regions.ENCSR637XSC.bed.gz"
...
for FOLD_NAME in "${FOLD_NAMES[@]}"; do
    ...
    BED_FILE="$INPUT_DIR/$FOLD_NAME/nonpeaks.all_input_regions.$FOLD_NAME.ENCSR637XSC.bed.gz"
