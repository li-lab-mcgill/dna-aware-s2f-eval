...
class CriticAdapter:
    ...
    This adapter expects a DNA mask (B, L) or (B, L, 1) and builds the full
    critic input mask internally (track channels are fully visible).
    """
    ...
    @staticmethod
    def _normalize_dna_mask(dna_mask: torch.Tensor) -> torch.Tensor:
        """
        Normalize DNA mask to (B, L) float tensor.
        Accepts (B, L) or (B, L, 1).
        """
        if dna_mask.dim() == 3 and dna_mask.size(-1) == 1:
            dna_mask = dna_mask.squeeze(-1)
        if dna_mask.dim() != 2:
            raise ValueError(f"dna_mask must be (B,L) or (B,L,1), got {dna_mask.shape}")
        return dna_mask
    ...
    def compute_logits_and_embeddings(
        ...
        *,
        ...
        dna_mask: torch.Tensor,  # (B, L) or (B, L, 1) DNA-only mask
        ...
        dna_mask : (B, L) or (B, L, 1)
            DNA-only mask; track channels are fully visible.
        ...
        dna_mask = self._normalize_dna_mask(dna_mask).to(device=dna.device, dtype=dna.dtype)
        if dna_mask.shape[0] != B or dna_mask.shape[1] != L:
            raise ValueError(
                f"dna_mask shape {dna_mask.shape} incompatible with track shape {(B, L, T)}"
            )

        dna_mask_4 = dna_mask.unsqueeze(-1).expand(B, L, 4)
        track_mask = torch.zeros(B, L, T, device=dna.device, dtype=dna.dtype)
        critic_input_mask = torch.cat([dna_mask_4, track_mask], dim=-1)
