...
class PeaksZarrDataset(Dataset):
    """
    Simplified PyTorch Dataset for pre-extracted genomics peak data.
    - Peaks only (no nonpeaks)
    ...
    def __init__(
        ...
        Args:
            Dataset parameters:
                peaks_zarr_path: Path to peaks zarr store
        ...
        if cache_dir is None:
            self.cache_dir = os.path.join(peaks_zarr_path, "_cache")
        else:
        ...
        print(f"=== Initializing Peaks Dataset ({split_name}) ===")
        ...
        split_file = np.load(temp)
        split_key = f"{fold_name}__{split_name}"
        if split_name in split_file:
            self.split_indices = split_file[split_name]
        else:
            self.split_indices = split_file[split_key]
        ...
        if need_data:
            ...
            with FileLock(seq_cache + '.lock'):
                if not os.path.exists(seq_cache):
                    ...
                    seq_data = np.asarray(self._navigate_to_dataset(self.peak_store, self.seq_dataset_path))
    ...
    def _load_single_rank_shared(self):
        ...
        seq_data = self._navigate_to_dataset(self.peak_store, self.seq_dataset_path)
    ...
    def update_mask_source(self, mask_dataset_path=None, mask_replicate_id=None):
        ...
        mask_arr = self._navigate_to_dataset(self.peak_store, self.mask_dataset_path)
        if mask_arr.ndim != 3:
    ...
    def __getitem__(self, idx):
        ...
        return seq_tensor, pred_tensor, exp_tensor, mask_tensor, "peak", identifier
...
def create_dataset(
    ...
    **kwargs,
    ...
    """
    Factory for creating simplified peaks-only dataset.
    ...
    Usage:
        ...
        ds = create_dataset(
            ...
            fold_name="fold_0",
            seq_dataset_path=["reference_dna", "dna_seq"],
            predicted_track_dataset_paths=[["fold_0__model_tracks", "atac"]],
            split_name="training"
        )
