...
class LightningSingleStemCriticPretrain(pl.LightningModule):
    """
    Lightning module for single-stem multi-head critic pretraining.
    ...
    - Keeps validation deterministic by not sampling tasks/masks inside the module.
    - Single stem (no regime routing), multiple heads (GT, S2F).
    """
    ...
    def validation_step(self, batch, batch_idx):
        # Deterministic validation: rely on val_batch_preparer not to use randomness
        prepared = self.val_batch_preparer(batch, device=self.device)
        losses = self(prepared)
        total = losses["total"]
        ...
        # Log scalar losses with explicit batch_size
        self.log("val/loss_total", total, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True, batch_size=batch_size)
        for name, val in losses.items():
            if name == "total":
                continue
            self.log(f"val/{name}", val, prog_bar=False, on_step=False, on_epoch=True, sync_dist=True, batch_size=batch_size)
        return total
