...
class BasePeakNonpeakZarrDataset(Dataset):
    ...
    def __init__(
        ...
        split_name: str,
        control_peaks_track_dataset_paths: Optional[List[List[str]]] = None,
        control_nonpeaks_track_dataset_paths: Optional[List[List[str]]] = None,
        ...
        control_track_width: Optional[int] = None,
        max_shift: int = 0,
        ...
        self.control_peaks_track_dataset_paths = control_peaks_track_dataset_paths or []
        self.control_nonpeaks_track_dataset_paths = control_nonpeaks_track_dataset_paths or []
        ...
        self.control_track_width = int(control_track_width) if control_track_width is not None else int(seq_width)
        ...
        # peak/nonpeak control track widths (kept separate from main/pred track widths)
        control_track_widths = []
        for path_list in self.control_peaks_track_dataset_paths:
            temp = self._navigate_to_dataset(self.peak_store, path_list)
            control_track_widths.append(temp.shape[1])
        if control_track_widths:
            assert len(set(control_track_widths)) == 1, "All provided peak control track datasets must have the same width"
            self.peak_control_track_zarr_width = control_track_widths[0]
        else:
            self.peak_control_track_zarr_width = self.peak_seq_zarr_width
        self.peak_control_track_zarr_summit = self.peak_control_track_zarr_width // 2

        control_track_widths = []
        for path_list in self.control_nonpeaks_track_dataset_paths:
            temp = self._navigate_to_dataset(self.nonpeak_store, path_list)
            control_track_widths.append(temp.shape[1])
        if control_track_widths:
            assert len(set(control_track_widths)) == 1, "All provided nonpeak control track datasets must have the same width"
            self.nonpeak_control_track_zarr_width = control_track_widths[0]
        else:
            self.nonpeak_control_track_zarr_width = self.nonpeak_seq_zarr_width
        self.nonpeak_control_track_zarr_summit = self.nonpeak_control_track_zarr_width // 2
        ...
        if self.shift_aug:
            ...
            assert self.control_track_width + 2*max_shift <= self.peak_control_track_zarr_width, f"(train, peak) control_track_width ({self.control_track_width}) + max_shift ({max_shift}) must be <= zarr_width ({self.peak_control_track_zarr_width})"
            assert self.control_track_width + 2*max_shift <= self.nonpeak_control_track_zarr_width, f"(train, nonpeak) control_track_width ({self.control_track_width}) + max_shift ({max_shift}) must be <= zarr_width ({self.nonpeak_control_track_zarr_width})"
        else:
            ...
            assert self.control_track_width <= self.peak_control_track_zarr_width, f"(train, peak) control_track_width ({self.control_track_width}) must be <= zarr_width ({self.peak_control_track_zarr_width})"
            assert self.control_track_width <= self.nonpeak_control_track_zarr_width, f"(train, nonpeak) control_track_width ({self.control_track_width}) must be <= zarr_width ({self.nonpeak_control_track_zarr_width})"
        ...
        print(
            f"Extraction windows: seq_width={seq_width}, "
            f"track_width={track_width}, control_track_width={self.control_track_width}"
        )
    ...
    def _load_ddp_safe_shared(self):
        ...
        cache_key = (
            ...
            tuple([tuple(t) for t in self.control_peaks_track_dataset_paths]),
            tuple([tuple(t) for t in self.control_nonpeaks_track_dataset_paths]),
            int(self.control_track_width),
        )
        ...
        if cache_key in self._global_data_cache:
            ...
            self.ctrl_peak_track_data = cached.get('ctrl_peak_track_data')
            self.ctrl_nonpeak_track_data = cached.get('ctrl_nonpeak_track_data')
            return
        ...
        cache_filename = (
            f"data_{self.fold_name}_{'_'.join(self.seq_dataset_path)}"
            f"_predP_{_tag(self.predicted_peaks_track_dataset_paths)}"
            f"_predN_{_tag(self.predicted_nonpeaks_track_dataset_paths)}"
            f"_expP_{_tag(self.experimental_peaks_track_dataset_paths)}"
            f"_expN_{_tag(self.experimental_nonpeaks_track_dataset_paths)}"
            f"_ctrlP_{_tag(self.control_peaks_track_dataset_paths)}"
            f"_ctrlN_{_tag(self.control_nonpeaks_track_dataset_paths)}"
            f"_ctrlW_{int(self.control_track_width)}.npy"
        )
        ...
        ctrl_peak_track_cache = os.path.join(self.cache_dir, f"ctrl_peak_track_{cache_filename}")
        ctrl_nonpeak_track_cache = os.path.join(self.cache_dir, f"ctrl_nonpeak_track_{cache_filename}")
        ...
        need_data = not all([
            ...
            os.path.exists(ctrl_peak_track_cache) if self.control_peaks_track_dataset_paths else True,
            os.path.exists(ctrl_nonpeak_track_cache) if self.control_nonpeaks_track_dataset_paths else True,
        ])
        ...
        if need_data:
            ...
            with FileLock(peak_seq_cache + '.lock'):
                if not os.path.exists(peak_seq_cache):
                    ...
                    if self.control_peaks_track_dataset_paths:
                        print("    Loading control peak track data...")
                        peak_list = []
                        for path_list in self.control_peaks_track_dataset_paths:
                            arr = np.asarray(self._navigate_to_dataset(self.peak_store, path_list))
                            peak_list.append(arr)
                        ctrl_peak = np.concatenate(peak_list, axis=2)
                        np.save(ctrl_peak_track_cache, ctrl_peak)
                        os.chmod(ctrl_peak_track_cache, 0o664)
                        print(f"      ✓ Saved control peak tracks: {ctrl_peak.shape} {ctrl_peak.dtype}")
                        del peak_list, ctrl_peak

                    if self.control_nonpeaks_track_dataset_paths:
                        print("    Loading control nonpeak track data...")
                        np_list = []
                        for path_list in self.control_nonpeaks_track_dataset_paths:
                            arr = np.asarray(self._navigate_to_dataset(self.nonpeak_store, path_list))
                            np_list.append(arr)
                        ctrl_non = np.concatenate(np_list, axis=2)
                        np.save(ctrl_nonpeak_track_cache, ctrl_non)
                        os.chmod(ctrl_nonpeak_track_cache, 0o664)
                        print(f"      ✓ Saved control nonpeak tracks: {ctrl_non.shape} {ctrl_non.dtype}")
                        del np_list, ctrl_non
        ...
        self.ctrl_peak_track_data = np.load(ctrl_peak_track_cache, mmap_mode='r') if os.path.exists(ctrl_peak_track_cache) else None
        self.ctrl_nonpeak_track_data = np.load(ctrl_nonpeak_track_cache, mmap_mode='r') if os.path.exists(ctrl_nonpeak_track_cache) else None
        ...
        self._global_data_cache[cache_key] = {
            ...
            'ctrl_peak_track_data': self.ctrl_peak_track_data,
            'ctrl_nonpeak_track_data': self.ctrl_nonpeak_track_data,
        }
        ...
        nonpeak_track_gb = sum([
            ...
            (self.ctrl_nonpeak_track_data.nbytes / (1024**3)) if self.ctrl_nonpeak_track_data is not None else 0,
        ])
        peak_track_gb += (self.ctrl_peak_track_data.nbytes / (1024**3)) if self.ctrl_peak_track_data is not None else 0
    ...
    def _load_single_rank_shared(self):
        ...
        # Load control tracks separately (can have different width from main/pred tracks)
        if self.control_peaks_track_dataset_paths:
            lst = []
            for path_list in self.control_peaks_track_dataset_paths:
                arr = np.asarray(self._navigate_to_dataset(self.peak_store, path_list))
                lst.append(arr)
                print(f"    ✓ Ctrl peak {path_list}: {arr.shape} {arr.dtype} = {arr.nbytes/(1024**3):.1f} GB")
            self.ctrl_peak_track_data = np.concatenate(lst, axis=2)
            del lst
        else:
            self.ctrl_peak_track_data = None
        ...
        if self.control_nonpeaks_track_dataset_paths:
            ...
            for path_list in self.control_nonpeaks_track_dataset_paths:
                arr = np.asarray(self._navigate_to_dataset(self.nonpeak_store, path_list))
                lst.append(arr)
                print(f"    ✓ Ctrl nonpeak {path_list}: {arr.shape} {arr.dtype} = {arr.nbytes/(1024**3):.1f} GB")
            self.ctrl_nonpeak_track_data = np.concatenate(lst, axis=2)
            del lst
        else:
            self.ctrl_nonpeak_track_data = None
        ...
        peak_track_gb = sum([
            (self.pred_peak_track_data.nbytes / (1024**3)) if self.pred_peak_track_data is not None else 0,
            (self.exp_peak_track_data.nbytes / (1024**3)) if self.exp_peak_track_data is not None else 0,
            (self.ctrl_peak_track_data.nbytes / (1024**3)) if self.ctrl_peak_track_data is not None else 0,
        ])
        nonpeak_track_gb = sum([
            (self.pred_nonpeak_track_data.nbytes / (1024**3)) if self.pred_nonpeak_track_data is not None else 0,
            (self.exp_nonpeak_track_data.nbytes / (1024**3)) if self.exp_nonpeak_track_data is not None else 0,
        ...
        if self.ctrl_peak_track_data is not None:
            print(f"    ✓ Ctrl peak tracks: {self.ctrl_peak_track_data.shape} {self.ctrl_peak_track_data.dtype}")
        if self.ctrl_nonpeak_track_data is not None:
            print(f"    ✓ Ctrl nonpeak tracks: {self.ctrl_nonpeak_track_data.shape} {self.ctrl_nonpeak_track_data.dtype}")
    ...
    def _apply_augmentations(
        ...
        *,
        ...
        forced_rc=None,
        return_params=False,
        track_width_override: Optional[int] = None,
        track_center_override: Optional[int] = None,
    ):
        """
        ...
        if track_data is not None:
            if track_center_override is not None:
                track_center_pos = int(track_center_override)
            else:
                track_center_pos = self.peak_track_zarr_summit if region_type == "peak" \
                    else self.nonpeak_track_zarr_summit
            track_width = int(track_width_override) if track_width_override is not None else int(self.track_width)
            ...
            start = track_center_pos - track_width // 2
    ...
    def __getitem__(self, idx):
        """Ultra-fast access; returns sequence, main/pred tracks, and control tracks separately.

        Output:
            seq_tensor: (seq_width, 4)
            pred_track_tensor: (track_width, T) or None
            exp_track_tensor: (track_width, T) or None
            control_track_tensor: (control_track_width, T_ctrl) or None
        ...
        if region_type == 'peak':
            ...
            ctrl_track_full = self.ctrl_peak_track_data[zarr_idx] if getattr(self, 'ctrl_peak_track_data', None) is not None else None
            control_summit = self.peak_control_track_zarr_summit
        else:
            ...
            ctrl_track_full = self.ctrl_nonpeak_track_data[zarr_idx] if getattr(self, 'ctrl_nonpeak_track_data', None) is not None else None
            control_summit = self.nonpeak_control_track_zarr_summit
        ...
        if ctrl_track_full is not None:
            _, ctrl_win = self._apply_augmentations(
                seq_data,
                ctrl_track_full,
                region_type,
                forced_shift=shift,
                forced_rc=rc_flip,
                track_width_override=self.control_track_width,
                track_center_override=control_summit,
            )
        ...
        if ctrl_win is not None:
            ctrl_tensor = torch.from_numpy(ctrl_win).float()
        else:
            ctrl_tensor = torch.empty(self.control_track_width, 0, dtype=torch.float32)

        return seq_tensor, pred_tensor, exp_tensor, ctrl_tensor, region_type, identifier
...
def create_dataset(
    ...
    split_name,
    control_peaks_track_dataset_paths=None,
    control_nonpeaks_track_dataset_paths=None,
    control_track_width=None,
    ...
    **kwargs,
    ...
    return BasePeakNonpeakZarrDataset(
        ...
        control_peaks_track_dataset_paths=control_peaks_track_dataset_paths,
        control_nonpeaks_track_dataset_paths=control_nonpeaks_track_dataset_paths,
        split_name=split_name,
        control_track_width=control_track_width,
