"""
TF_ChIP-specific masked peak-nonpeak replicate dataset for evaluation.

Extends TF_ChIP's BasePeakNonpeakZarrDataset to add precomputed replicate masking.
"""
import zarr
import torch
import numpy as np
...
from .tfchip_base_zarr_dataset import BasePeakNonpeakZarrDataset
...
class MaskedPeakNonpeakReplicateZarrDataset(BasePeakNonpeakZarrDataset):
    ...
    Extends the TF_ChIP BasePeakNonpeakZarrDataset (which supports different seq_width and track_width)
    ...
    - Handles control tracks (TF_ChIP specific)
    """
    ...
    def __init__(
        ...
        # Track paths - using predicted/experimental naming
        ...
        Args:
            Dataset parameters:
                ...
                control_peaks_track_dataset_paths: List of lists for control peak tracks
                control_nonpeaks_track_dataset_paths: List of lists for control nonpeak tracks
            ...
            Sequence and track parameters:
                ...
                control_track_width: Width of control tracks (defaults to seq_width)
                max_shift: Maximum shift in bp for shift augmentation
                rc_aug: Enable reverse complement augmentation
                rc_strand_flip: Flip strand channels on RC
    ...
    def __getitem__(self, idx):
        ...
        # Parent returns: (seq_tensor, pred_tensor, exp_tensor, ctrl_tensor, region_type, identifier)
        seq_tensor, pred_tensor, exp_tensor, ctrl_tensor, region_type, identifier = super().__getitem__(idx)
        ...
        # Return separate tensors plus position mask (excluding ctrl_tensor for bias model eval)
...
def create_dataset(
    ...
    fold_name,
    seq_dataset_path,
    # Track paths - support predicted/experimental format
