...
from .unet.masked_input_layer import MaskedInputConv2DLayer
...
class MaskedGenomeUNet(nn.Module):
    """
    UNet for multimodal masked genome language model

    Architecture:
    Input: (B, 2, L, T) -> InputConv2D -> (B, input_channels, L)
    Encoder: DownBlocks with skip connections
    Bottleneck: Transformer with RoPE + GQA
    Decoder: UpBlocks with skip connections
    Output: (B, L, T) - reconstructed values (no mask channel)
        For tracks it is log1p of the count of the track at each position
    ...
    def __init__(self,
        ...
        self.final_out_channels = 4  # predicting for only DNA
        ...
        self.final_conv = FinalOutputBlock(
            ...
            out_channels=self.final_out_channels,  # DNA only (4 channels)
            num_groups=num_groups,
            dropout=dropout
        )
    ...
    def forward(self,
        ...
        # Input convolution: (B, 2, L, T) -> (B, input_conv_channels, L)
        x = self.input_conv(x)
    ...
    def _get_architecture_summary(self) -> str:
        ...
        summary = []
        summary.append(f"Input: (B, 2, L, {self.T})")
        summary.append(f"InputConv2D: -> (B, {self.input_conv_channels}, L)")
    ...
    @classmethod
    def create_mini_model(cls, T: int = 42, **kwargs) -> 'MaskedGenomeUNet':
    ...
    @classmethod
    def create_medium_model(cls, T: int = 42, **kwargs) -> 'MaskedGenomeUNet':
    ...
    @classmethod
    def create_large_model(cls, T: int = 42, **kwargs) -> 'MaskedGenomeUNet':
