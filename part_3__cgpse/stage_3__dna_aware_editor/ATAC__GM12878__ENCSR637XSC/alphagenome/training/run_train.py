...
# -----------------------------------------------------------------------------
# Final DNA-aware editor training script
# -----------------------------------------------------------------------------
# This trains:
#   - DNAAwareEditorAugmentedDAE (trainable editor on frozen DNA-free DAE)
# using:
#   - pretrained DNAFreeDAE (frozen)
...
# Final DAE + editor model
from utils.final_debiaser_v2.model.dna_aware_editor import DNAAwareEditorAugmentedDAE

# Orchestrator, adapter, losses for final DNA-aware editor
from utils.final_debiaser_v2.orchestrator.critic_adapter import CriticAdapter
from utils.final_debiaser_v2.orchestrator.editor_orchestrator import TrainingOrchestrator
from utils.final_debiaser_v2.orchestrator.editor_lightning import LightningWrapper
...
CRITIC_MODEL_PATH = (
    ...
    "alphagenome__predicted/1000bp__mask_10to100pct/seed_42/lightning_logs/version_0/"
    "checkpoints/best_val_100_GT_loss-epoch=567-step=210728.critic.pt"
)

DAE_MODEL_PATH = (
    "/home/mcb/users/dcakma3/multi_res_bench_v2/multimodal_masked_contrastive_main/"
    "manuscript_experiments/section_3_v2__Debiasing/workspace/runs/6_1__final_debiaser/"
    "ATAC/GM12878__ENCSR637XSC/dna_free_dae/alphagenome__predicted/"
    ...
    "staged_checkpoints/best_val_100_total_loss-epoch=112-step=47121.dae.pt"
)
...
def load_frozen_dae_from_checkpoint() -> torch.nn.Module:
    """
    Load a pretrained DNA-free DAE object and freeze it.
    """
    if not os.path.exists(DAE_MODEL_PATH):
        raise FileNotFoundError(f"Missing DAE checkpoint: {DAE_MODEL_PATH}")

    dae = torch.load(DAE_MODEL_PATH, map_location="cpu", weights_only=False)
    dae.eval()
...
def main(config_dict):
    ...
    """ Init frozen DAE + trainable editor """
    dae = load_frozen_dae_from_checkpoint()
    main_process_print(f"Frozen DAE loaded with {sum(p.numel() for p in dae.parameters()):,} parameters")

    dae_in_channels = int(getattr(dae, "in_channels", getattr(dae, "track_in_channels", T_tracks)))
    if T_tracks != dae_in_channels:
        raise ValueError(
            f"Loaded DAE expects {dae_in_channels} track channels, got {T_tracks}"
        )

    dae_latent_channels = getattr(dae, "conv_latent_channels", None)
    if dae_latent_channels is None:
        dae_latent_channels = getattr(dae, "track_conv_latent_channels")
    dae_latent_channels = int(dae_latent_channels)
    dae_bottleneck_channels = int(getattr(dae, "bottleneck_channels"))

    reencoder_kwargs = config_dict.reencoder.to_dict()
    reencoder_kwargs["track_in_channels"] = T_tracks

    if int(reencoder_kwargs["track_conv_latent_channels"]) != dae_latent_channels:
        raise ValueError(
            "Reencoder track_conv_latent_channels must match DAE conv_latent_channels "
            f"({reencoder_kwargs['track_conv_latent_channels']} vs {dae_latent_channels})"
        )
    if int(reencoder_kwargs["dna_latent_channels"]) != dae_latent_channels:
        raise ValueError(
            "Reencoder dna_latent_channels must match DAE conv_latent_channels "
            f"({reencoder_kwargs['dna_latent_channels']} vs {dae_latent_channels})"
        )
    if int(reencoder_kwargs["bottleneck_channels"]) != dae_bottleneck_channels:
        raise ValueError(
            "Reencoder bottleneck_channels must match DAE bottleneck_channels "
            f"({reencoder_kwargs['bottleneck_channels']} vs {dae_bottleneck_channels})"
        )

    editor_kwargs = config_dict.editor.to_dict()
    if int(editor_kwargs["context_channels"]) != dae_latent_channels:
        raise ValueError(
            "Editor context_channels must match DAE conv_latent_channels "
            f"({editor_kwargs['context_channels']} vs {dae_latent_channels})"
        )
    if int(editor_kwargs["bottleneck_channels"]) != dae_bottleneck_channels:
        raise ValueError(
            "Editor bottleneck_channels must match DAE bottleneck_channels "
            f"({editor_kwargs['bottleneck_channels']} vs {dae_bottleneck_channels})"
        )

    editor = DNAAwareEditorAugmentedDAE(
        dae=dae,
        editor_kwargs=editor_kwargs,
        reencoder_kwargs=reencoder_kwargs,
    )
    main_process_print(
        f"Editor initialized with {sum(p.numel() for p in editor.parameters()):,} parameters "
        f"({sum(p.numel() for p in editor.parameters() if p.requires_grad):,} trainable)"
    )
    ...
    coeff_suffix = str(config_dict.task.coeff_suffix)
    fidelity_dir = str(config_dict.task.fidelity_dir)
    config_dict.task.experiment_dir = osp.join(
        MANUSCRIPT_SECTION_3_V2_DIR,
        "workspace",
        "runs",
        "7_1__final_dna_aware_editor",
        config_dict.task.assay_type,
        config_dict.task.assay_experiment_identifier,
        "dna_aware_editor",
        "alphagenome__predicted",
        (
            f"{int(config_dict.task.bucket_input_len)}bp__mask_rco_"
            f"{int(config_dict.task.train_rco_min * 100)}to{int(config_dict.task.train_rco_max * 100)}pct"
            f"__rci_max_{int(config_dict.task.train_rci_max * 100)}pct"
        ),
        f"latentC{dae_latent_channels}__{coeff_suffix}",
        f"editorH{config_dict.editor.hidden_channels}",
        fidelity_dir,
...
if __name__ == "__main__":
    ...
    config_dict.task.task_type = "final_dna_aware_editor_training"

    """ Model / checkpoint config """
    config_dict.model = ConfigDict()
    config_dict.model.name = "FinalDNAAwareEditorTraining"

    """ Reencoder config (track + masked DNA -> context) """
    config_dict.reencoder = ConfigDict()
    config_dict.reencoder.track_base_channels = 32
    config_dict.reencoder.track_conv_latent_channels = 64
    config_dict.reencoder.track_encoder_kernel_size = 5
    config_dict.reencoder.track_stem_kernel_size = 23
    config_dict.reencoder.track_down_kernel_size = 5
    config_dict.reencoder.dna_num_modalities = 4
    config_dict.reencoder.dna_base_channels = 32
    config_dict.reencoder.dna_latent_channels = 64
    config_dict.reencoder.dna_stem_kernel_size = 23
    config_dict.reencoder.dna_down_kernel_size = 5
    config_dict.reencoder.dna_padding_mode = "same"
    config_dict.reencoder.dna_mask_aware_padding = True
    config_dict.reencoder.bottleneck_channels = 32
    config_dict.reencoder.bottleneck_depth = 1
    config_dict.reencoder.bottleneck_heads = 4
    config_dict.reencoder.bottleneck_dim_head = None
    config_dict.reencoder.bottleneck_kv_heads = None
    config_dict.reencoder.bottleneck_attn_dropout = 0.1
    config_dict.reencoder.bottleneck_proj_dropout = 0.1
    config_dict.reencoder.bottleneck_ff_mult = 2.0
    config_dict.reencoder.bottleneck_ff_dropout = 0.1
    config_dict.reencoder.bottleneck_rope_fraction = 0.5
    config_dict.reencoder.bottleneck_rope_theta = 20000.0
    config_dict.reencoder.bottleneck_swiglu_use_layernorm = False

    """ Editor config (FiLM at bottleneck code) """
    config_dict.editor = ConfigDict()
    config_dict.editor.context_channels = 64
    config_dict.editor.bottleneck_channels = 32
    config_dict.editor.hidden_channels = 64
    config_dict.editor.use_layernorm = False
    ...
    config_dict.data.predicted_peaks_track_dataset_paths = [
        [
            f"fold_agnostic__model_based_tracks",
    ...
    # Experiment directory components (filled in main after DAE load).
    config_dict.task.model_name = config_dict.model.name
    config_dict.task.coeff_suffix = _build_count_coeff_suffix(count_coeff_config)
    config_dict.task.fidelity_dir = fidelity_dir
    config_dict.task.bucket_input_len = BUCKET_INPUT_LEN
    config_dict.task.train_rco_min = train_rco_min
    config_dict.task.train_rco_max = train_rco_max
    config_dict.task.train_rci_max = train_rci_max
